...
export class AuthController {
  ...
  async login(
    ...
    const { access_token, refresh_token, consent_required } = await this.authService.login(auth);
  ...
  async refreshToken(@Req() req: Request): Promise<JwtToken> {
    ...
    const { access_token, consent_required } = await this.authService.refresh(refreshToken);
    return { access_token, consent_required };
  }
