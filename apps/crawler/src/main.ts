...
async function bootstrap() {
  ...
  try {
      ...
      app
        ...
        .pipe(
          ...
          map(async (notice) => ({
            prev: await app
              .get(CrawlerService)
              .checkCrawlData(notice.meta.link),
            notice: notice,
          })),
          concatMap(identity),
          toArray(),
    ...
    logger.log(
      `All tasks completed successfully in ${duration}ms. Sending 'UP' heartbeat.`,
    );
    const successUrl = `${configService.CRAWLING_UPTIME_URI}?status=up&msg=OK&ping=${duration}`;
    await fetch(successUrl);
  } catch (error) {
    const duration = Date.now() - startTime;
    const errorMessage =
      error instanceof Error ? error.message : 'Unknown Error'; //url로 보내야 되서 이렇게 해둠
    logger.error(
      `An error occurred: ${errorMessage}. Sending 'DOWN' heartbeat.`,
    );
    const faireUrl = `${configService.CRAWLING_UPTIME_URI}?status=down&msg=${errorMessage}&ping=${duration}`;
    await fetch(faireUrl);
  }

  logger.log('Closing the application context.');
  await app.close();
}
bootstrap();
